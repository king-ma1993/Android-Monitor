...
import com.myl.fastplugin.common.utils.CalendarUtils;
...
public class HtmlReporter {
    ...
    private static final String CSS = ".DEBUG { color: black; } .INFO { color: black; } .ERROR { color: red; } "
        + ".WARNING { color: orange; } table { border: 1px solid #e3e6e8; border-collapse: collapse; display: table; "
        + "} th { background-color: #189AD6; color: #fff; } .side-bar { width: 100px; position: fixed; bottom:100px; "
        + "right: 100px; z-index: 100; } .side-bar a { width: 66px; height: 66px; display: inline-block; "
        + "background-color: #ddd; margin-outside: 100px; } .side-bar .icon-chat {background-position: 0 -130px;"
        + "position: relative;} .side-bar a:hover { background-color: #669fdd; }";
    ...
    /**
     * register a HtmlFragmentProvider,which will be use to create a piece of HTML code when {@link #createHtmlReporter}
     * is called
     *
    ...
    public synchronized void init(String htmlFileDir, String title, String appPackageName, String versionName,
        String versionCode) {
    ...
    private synchronized void appendHtml(Appendable appendable) throws IOException {
        ...
        appendable.append("<!DOCTYPE html>")
            ...
            .append("Report Generated Time: ")
            .append(CalendarUtils.getDateAndTimeString(System.currentTimeMillis(), true)).append("<br>")
